    ...
    <div class="row justify-content-center">
        <div class="col-8">
            <form name="editForm" role="form" novalidate v-on:submit.prevent="save()" >
                <h2 id="balbaursakApp.product.home.createOrEditLabel" v-text="$t('balbaursakApp.product.home.createOrEditLabel')">Create or edit a Product</h2>
                <div>
                    ...
                    <div class="form-group">
                        <label class="form-control-label" v-text="$t('balbaursakApp.product.name')" for="product-name">Name</label>
                    ...
                    <div class="form-group">
                        <label class="form-control-label" v-text="$t('balbaursakApp.product.unit')" for="product-unit">Unit</label>
                        <input type="text" class="form-control" name="unit" id="product-unit"
                            :class="{'valid': !$v.product.unit.$invalid, 'invalid': $v.product.unit.$invalid }" v-model="$v.product.unit.$model"  required/>
                        <div v-if="$v.product.unit.$anyDirty && $v.product.unit.$invalid">
                            <small class="form-text text-danger" v-if="!$v.product.unit.required" v-text="$t('entity.validation.required')">
                                This field is required.
                            </small>
                        </div>
                    </div>
