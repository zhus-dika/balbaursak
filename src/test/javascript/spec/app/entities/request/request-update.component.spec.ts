/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import Router from 'vue-router';

import AlertService from '@/shared/alert/alert.service';
import * as config from '@/shared/config/config';
import RequestUpdateComponent from '@/entities/request/request-update.vue';
import RequestClass from '@/entities/request/request-update.component';
import RequestService from '@/entities/request/request.service';

import RequestpointService from '@/entities/requestpoint/requestpoint.service';

const localVue = createLocalVue();

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
const router = new Router();
localVue.use(Router);
localVue.component('font-awesome-icon', {});

describe('Component Tests', () => {
  describe('Request Management Update Component', () => {
    let wrapper: Wrapper<RequestClass>;
    let comp: RequestClass;
    let requestServiceStub: SinonStubbedInstance<RequestService>;

    beforeEach(() => {
      requestServiceStub = sinon.createStubInstance<RequestService>(RequestService);

      wrapper = shallowMount<RequestClass>(RequestUpdateComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: {
          alertService: () => new AlertService(store),
          requestService: () => requestServiceStub,

          requestpointService: () => new RequestpointService(),
        },
      });
      comp = wrapper.vm;
    });

    describe('save', () => {
      it('Should call update service on save for existing entity', async () => {
        // GIVEN
        const entity = { id: 123 };
        comp.request = entity;
        requestServiceStub.update.resolves(entity);

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(requestServiceStub.update.calledWith(entity)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });

      it('Should call create service on save for new entity', async () => {
        // GIVEN
        const entity = {};
        comp.request = entity;
        requestServiceStub.create.resolves(entity);

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(requestServiceStub.create.calledWith(entity)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });
    });
  });
});
